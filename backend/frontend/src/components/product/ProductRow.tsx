...
// ── Compact card used inside the horizontal row ───────────────────────────────
const CompactProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  const base =
    ...
    resolveLocalUrl(product.imageUrl) ||
    resolveProductImage(product.slug, product.name)
  ...
  return (
    <Link
      ...
      className="group flex-shrink-0 w-[140px] sm:w-[152px] bg-white rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden flex flex-col border border-stone-100 hover:border-forest-200"
    >
      {/* Image area */}
      <div className="relative w-full aspect-square bg-gradient-to-br from-emerald-50 to-stone-50 overflow-hidden">
        ...
        {product.isOrganic && (
          <span className="absolute top-1.5 left-1.5 inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-label font-bold bg-forest-700/90 text-white leading-tight backdrop-blur-sm">
            <Leaf className="w-2 h-2" strokeWidth={2.5} />
            ORG
          </span>
        )}
      ...
      <div className="p-2 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          <span className="text-sm font-display font-bold text-forest-700">
            ₹{product.price}
          </span>
          {product.originalPrice && product.originalPrice > product.price && (
            <span className="text-[10px] text-stone-400 line-through font-body">
        ...
        {/* Savings badge */}
        {savingsAmt > 0 && (
          <span className="text-[9px] font-label font-semibold text-forest-600 leading-tight">
            ₹{savingsAmt} OFF
          </span>
        )}
...
// ── ProductRow — main exported component ──────────────────────────────────────
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-3 px-4 sm:px-0">
        ...
        <Link
          to={seeAllLink}
          className="flex items-center gap-0.5 text-sm font-label font-semibold text-forest-600 hover:text-forest-800 transition-colors"
        >
      ...
      <div className="relative">
        ...
        {canScrollLeft && (
          <button
            onClick={scrollLeft}
            className="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 w-8 h-8 items-center justify-center bg-white border border-stone-200 rounded-full shadow-md hover:shadow-lg hover:border-forest-300 transition-all duration-150"
          >
        ...
        {canScrollRight && (
          <button
            onClick={scrollRight}
            className="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 w-8 h-8 items-center justify-center bg-white border border-stone-200 rounded-full shadow-md hover:shadow-lg hover:border-forest-300 transition-all duration-150"
          >
        ...
        <div
          ...
          className="flex gap-3 overflow-x-auto pb-2 px-4 sm:px-0 scrollbar-hide scroll-smooth snap-x snap-mandatory"
        >
          {loading
